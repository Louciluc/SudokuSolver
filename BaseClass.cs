...
namespace Sudoku {
	public partial class SudokuGrid {
		...
		{
			Action<SudokuGrid, List<SudokuCell>>? printEmptyCellsStats = 
				printLogs ? ((SudokuGrid grid, List<SudokuCell> a) => Console.WriteLine("Empty cells at the moment: " + a.Count.ToString())) : null;
			Action<SudokuGrid, SudokuCell, List<int>, int>? beforeRecursiveSolve = 
				printLogs ? ((SudokuGrid grid, SudokuCell chosenCell, List<int> optionsToTry, int tryNumber) =>
			{
			...
			Action<SudokuGrid, bool, bool, SudokuCell, List<int>, int>? afterRecursiveSolve = 
				printLogs ? ((SudokuGrid grid, bool worked, bool atLeastOneWorked, SudokuCell chosenCell, List<int> optionsToTry, int tryNumber) => {
