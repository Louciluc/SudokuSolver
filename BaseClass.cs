...
namespace Sudoku
{
    public partial class SudokuGrid
    {
        ...
        {
            get => BoxSize.Item1 * BoxSize.Item2;
        }
        public readonly (int, int) BoxSize;
        public (int, int) BoxCount
        {
            get { return (BoxSize.Item2, BoxSize.Item1); }
        }
        public SudokuCell[,] Grid;

        private delegate void AllAction(
            int x,
            int y,
        ...
        public List<int> AllValues1ToLast
        {
            get => [.. Enumerable.Range(1, BoxSize.Item1 * BoxSize.Item2)];
        }
        public List<int[,]> AllPossibleSolutions
        {
            get
            {
        ...
        public SudokuGrid(int?[,] _grid)
        {
            ...
            if (Length != _grid.GetLength(1))
                throw new ArgumentException("Input grid isnt quadratic!", _grid.GetType().Name);
            ...
            AllAction lambda = (int x, int y, SudokuGrid sudokuGrid, params object[] parameters) =>
            {
                ...
                sudokuGrid.Grid[x, y] = new SudokuCell(
                    (parameters[0] as int?[,])[x, y],
                    (x, y),
                    isdefright
                );
            };
        ...
        {
            if ((int)Math.Sqrt(Length) == Math.Sqrt(Length))
                return ((int)Math.Sqrt(Length), (int)Math.Sqrt(Length));
            ...
            for (int i = 1; i < Length; i++)
            {
                ...
                {
                    if (i * j == Length)
                        possibleMatches.Add(new(i, j));
                }
            }

            possibleMatches.Sort();
            if (possibleMatches.Count == 0)
                throw new Exception("No BoxSize Found!");
        ...
        private struct BoxSizeComparer : IComparable<BoxSizeComparer>
        {
            ...
            public BoxSizeComparer(int x, int y)
            {
                Value = (x, y);
            }
            ...
            {
                return Math.Abs(this.Value.Item1 - this.Value.Item2)
                    .CompareTo(Math.Abs(other.Value.Item1 - other.Value.Item2));
            }
        }

        private static void loopAll(
            AllAction? actionAll,
            XAction? actionX,
            int xLimit,
            int yLimit,
        ...
        {
            List<SudokuCell> DuplicateCheck(
                List<SudokuCell> listWithDoubles,
                ref List<(int, int)> errorList,
                ref bool isvalid
            )
            {
                // https://stackoverflow.com/questions/18547354/c-sharp-linq-find-duplicates-in-list
                List<SudokuCell> duplicates = listWithDoubles
                    .GroupBy(x => x.Value)
                    .Where(g => g.Count() > 1)
                ...
                foreach (SudokuCell dup in duplicates)
                    errorList.Add(dup.Position);
            ...
            for (int x = 0; x < BoxCount.Item1; x++)
            {
                for (int y = 0; y < BoxCount.Item2; y++)
                {
                    ...
                    List<SudokuCell> duplicates = DuplicateCheck(
                        list,
                        ref posOfErrors,
                        ref isvalid
                    );
        ...
        {
            return (
                posInBox.Item1 + boxWherePosIsIn.Item1 * BoxSize.Item1,
                posInBox.Item2 + boxWherePosIsIn.Item2 * BoxSize.Item2
            );
        }
        ...
        private SudokuGrid GetSudokuGridWithColours(int?[,] numbers)
        {
            ...
            AllAction checkIfGiven = (int x, int y, SudokuGrid grid, params object[] paramteers) =>
                sgrid.Grid[x, y].IsDefRight = this.Grid[x, y].IsDefRight;
        ...
        private int?[,] GetNullerableGrid(int[,] input)
        {
            ...
            AllAction castToNullable = (
                int x,
                int y,
                SudokuGrid grid,
                params object[] parameters
            ) => nullableGrid[x, y] = input[x, y];
        ...
        public List<int> FindPossibleOptionsForCell(SudokuCell cell)
        {
            ...
            for (int i = 0; i < TotalSize; i++)
            {
                if (i == position.Item2)
                    continue;
                if (Grid[position.Item1, i].Value != null)
                    vertical.Add((int)Grid[position.Item1, i].Value);
            }
            sw_v.Stop();
            Console.WriteLine($"Time elapsed for vertical: {sw_v.Elapsed.TotalNanoseconds}");
            ...
            for (int i = 0; i < TotalSize; i++)
            {
                if (i == position.Item1)
                    continue;
                if (Grid[i, position.Item2].Value != null)
                    horizontal.Add((int)Grid[i, position.Item2].Value);
            }
            sw_h.Stop();
            Console.WriteLine($"Time elapsed for horizontal: {sw_h.Elapsed.TotalNanoseconds}");
            ...
            for (int a = 0; a < BoxSize.Item1; a++)
            {
                for (int b = 0; b < BoxSize.Item2; b++)
                {
                    ...
                    if (a == posInBox.Item1 && b == posInBox.Item2)
                        continue;
                    ...
                    if (Grid[globalPos.Item1, globalPos.Item2].Value != null)
                        box.Add((int)Grid[globalPos.Item1, globalPos.Item2].Value);
                }
            }
            sw_b.Stop();
            Console.WriteLine($"Time elapsed for box: {sw_b.Elapsed.TotalNanoseconds}");

            sw.Stop();
            Console.WriteLine("Time elapsed for getting options: {0}", sw.Elapsed.TotalNanoseconds);

            Stopwatch sw_sort = new();
            sw_sort.Start();
            ...
            sw_sort.Stop();
            Console.WriteLine($"Time elapsed for sorting: {sw_sort.Elapsed.TotalNanoseconds}");
        ...
        )
        {
            ...
            sw.Stop();
            Console.WriteLine(
                "Elapsed ticks for setting all cells: {0}",
            ...
            loopAll(
                (int x, int y, SudokuGrid grid, params object[] param) =>
                {
                    if (grid.Grid[x, y].Value == null)
                        emptyCells.Add(grid.Grid[x, y]);
                },
                null,
                TotalSize,
                TotalSize,
            ...
            if (emptyCells[0].OptionsLeft.Count == 0)
                return false;
            ...
            foreach (int tryNumber in optionsToTry)
            {
                ...
                bool worked = RecursiveSolve(
                    printEmptyCellsStats,
                    beforeRecursiveSolve,
                    afterRecursiveSolve,
                    afterAllOptionsTried,
                    calcAllSolutions
                );

                afterRecursiveSolve?.Invoke(
                    this,
                    worked,
                    atLeastOneWorked | worked,
                    chosenCell,
                    optionsToTry,
                    tryNumber
                );
        ...
        public bool RecursiveSolve(
            bool calcAllSolutions = true,
            Action<SudokuGrid, List<SudokuCell>>? printEmptyCellsStats = null,
            Action<SudokuGrid, SudokuCell, List<int>, int>? beforeRecursiveSolve = null,
            Action<SudokuGrid, bool, bool, SudokuCell, List<int>, int>? afterRecursiveSolve = null,
            Action<SudokuGrid, bool, SudokuCell>? afterAllOptionsTried = null
        )
        {
            ...
            if (listOfAllCells.Count < 10)
            {
                Console.WriteLine(
                    "The input Sudoku hadnt had enough filled Cells."
                        + ToString()
                        + "\nThe minimus amount of filled cells in a 9x9 Sudoku is 17 (filled cells) to get a sudoku with only one solution"
                );
                return false;
            }
        ...
        {
            Action<SudokuGrid, List<SudokuCell>>? printEmptyCellsStats = printLogs
                ? (
                    (SudokuGrid grid, List<SudokuCell> a) =>
                        Console.WriteLine("Empty cells at the moment: " + a.Count.ToString())
                )
                : null;
            Action<SudokuGrid, SudokuCell, List<int>, int>? beforeRecursiveSolve = printLogs
                ? (
                    (
                        SudokuGrid grid,
                        SudokuCell chosenCell,
                        List<int> optionsToTry,
                        int tryNumber
                    ) =>
                    {
                        Console.WriteLine(
                            $"Trying {tryNumber} for cell {chosenCell.Position} with options left: [{string.Join(", ", optionsToTry)}]"
                        );
                ...
                printLogs
                    ? (
                        ...
                        ) =>
                        {
                            if (worked)
                                Console.WriteLine(
                                    $"The try with {tryNumber} for cell {chosenCell.Position} worked!"
                                );
                            else
                                Console.WriteLine(
                                    $"The try with {tryNumber} for cell {chosenCell.Position} didnt work!"
                                );
                        }
                    )
                    : null;
            Action<SudokuGrid, bool, SudokuCell>? afterAllOptionsTried = printLogs
                ? (
                    (SudokuGrid grid, bool atLeastOneWorked, SudokuCell chosenCell) =>
                    {
                        if (atLeastOneWorked)
                        {
                            Console.WriteLine(
                                $"At least one option for cell {chosenCell.Position} worked, going back to previous cell"
                            );
                        }
                        else
                        {
                            Console.WriteLine(
                                $"No option for cell {chosenCell.Position} worked, backtracking..."
                            );
                        }
                    }
                )
